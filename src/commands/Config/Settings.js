...
class SettingsCommand extends Command {
    constructor() {
      super('settings', {
        ...
        category: 'Config',
        description: 'Various setting configurations',
        args: [{
          id: 'option',
          type: 'string',
          default: null
    ...
    async exec(message, args) {
      ...
      if (args.option === 'prefix') {
        ...
        if (args.prefix.length > 6) return message.channel.send(`❌ **Your prefix can\'t be longer than 6 characters**`);
        await this.client.settings.set(message.guild.id, 'prefix', args.prefix);
        embed.setTitle(`Prefix updated`)
             .setDescription(`✅ **Your prefix has been updated to** \`${args.prefix}\``)
        return message.channel.send(embed);
      }
      if (args.option === 'wc') {
        if (!args.prefix) {
      ...
      if (args.option === 'jm') {
        ...
        if (msg.length < 1) {
          embed.setTitle(`Join Message`)
               .setDescription(`Update the join message with \`${this.client.settings.get(message.guild.id, 'prefix')}settings joinmessage message\``)
