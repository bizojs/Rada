...
class ReactionHandler extends ReactionCollector {
	constructor(message, filter, options, display, emojis) {
		...
		this.on('end', () => {
			if (this.reactionsDone && !this.message.deleted && message.guild.me.permissions.has('MANAGE_MESSAGES')) this.message.reactions.removeAll();
		});
	}
	first() {
		this.currentPage = 0;
	...
	async _queueEmojiReactions(emojis) {
		...
		if (this.ended && message.guild.me.permissions.has('MANAGE_MESSAGES')) return this.message.reactions.removeAll();
