const paginate = async (msg, pages, emojiList = ['⏪', '742375771913453628', '🗑️', '742375779656269914', '⏩'], timeout = 120000) => {
    ...
    typeof pages[page] === 'object' ? 
        await msg.util.send(pages[page].setFooter(`Page ${page + 1} of ${pages.length} | Requested by ${msg.author.username}`)) :
        await msg.util.send(`${pages[page]}\n\n*Page ${page + 1} of ${pages.length}*`);
	for (const emoji of emojiList) await curPage.react(emoji);
    const reactionCollector = curPage.createReactionCollector((reaction, user) => 
    emojiList.includes(reaction.emoji.name) || emojiList.includes(reaction.emoji.id) && !user.bot && user.id === msg.author.id, { time: timeout });
