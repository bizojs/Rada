...
class CustomLog {
	...
	verbose = (user, command, channel, guild) => {
		console.log(`${colors.bgBlue(`[${new Date().toLocaleTimeString()}]`)} ${colors.black.bgWhite(user)} ${colors.black.bgRed(command)} ${colors.black.bgYellow(channel)} ${colors.black.bgGreen(guild)}`)
	}
}
