	...
	class RadaMember extends Member {
		constructor(...args) {
			...
			this.addWarn = async (moderator, reason) => {
				...
				let duplicateCount = db.filter(warn => warn === reason).length;
                                let infraction = this.settings.get(this.id, 'warnings', array).includes(reason) ?
                                    `Warned by __${moderator.tag}__ for **${reason} #${duplicateCount + 1}**` :
                                    `Warned by __${moderator.tag}__ for **${reason}**`
				if (db.length < 1) {
